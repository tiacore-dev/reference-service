...
async def test_add_city(test_app: AsyncClient, jwt_token_admin: dict):
    ...
    data = {"city_name": "Test City", "region": "Test Region", "code": "66666", "external_id": "1111", "timezone": 0}
...
async def test_edit_city(test_app: AsyncClient, jwt_token_admin: dict, seed_city: City):
    ...
    response = await test_app.patch(f"/api/cities/{seed_city.id}", headers=headers, json=data)
...
@pytest.mark.asyncio
async def test_delete_city(test_app: AsyncClient, jwt_token_admin: dict, seed_city: City):
...
@pytest.mark.asyncio
async def test_get_cities(test_app: AsyncClient, jwt_token_admin: dict, seed_city: City):
