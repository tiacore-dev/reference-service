...
@pytest.mark.asyncio
async def test_add_warehouse(test_app: AsyncClient, jwt_token_admin: dict):
    """Тест добавления нового промпта."""
    headers = {"Authorization": f"Bearer {jwt_token_admin['access_token']}"}
    data = {"warehouse_name": "Test Warehouse", "company_id": str(uuid4())}
...
@pytest.mark.asyncio
async def test_edit_warehouse(
    test_app: AsyncClient, jwt_token_admin: dict, seed_warehouse: Warehouse
):
    ...
    response = await test_app.patch(
        f"/api/warehouses/{seed_warehouse.id}", headers=headers, json=data
    )
...
async def test_view_warehouse(
    ...
    response = await test_app.get(
        f"/api/warehouses/{seed_warehouse.id}", headers=headers
    )
...
async def test_get_warehouses(
    ...
    assert str(seed_warehouse.id) in warehouse_ids, (
